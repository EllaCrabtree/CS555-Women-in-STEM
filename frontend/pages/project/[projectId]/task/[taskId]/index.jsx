...
const Task = () => {
	...
	const completeSubtask = async (subtaskID) => {
		...
		for (let i = 0; i < taskData.SubTasks.length; i++) {
			...
			if (tempData.SubTasks[i].SubTaskID == subtaskID) {
				tempData.SubTasks[i].Completion_Status =
					(tempData.SubTasks[i].Completion_Status - 1) * -1;
	...
	if (error) {
		...
		const subtaskElements_NotCompleted = taskData.SubTasks.filter(
			(subtask) => subtask.Completion_Status == 0
		).map((subtask) => {
			return BuildSubTaskCard(subtask);
		});

		const subtaskElements_Completed = taskData.SubTasks.filter(
			(subtask) => subtask.Completion_Status == 1
		).map((subtask) => {
			return BuildSubTaskCard(subtask);
		});
		...
		if (imageError) {
			if (!spanish) {
				...
				imageErrorContent = <p> Sube una foto más pequeña </p>;
			}
		}
		...
		if (imageElements.length == 0) {
			if (!spanish) {
				...
				imageContent = "No hay imágenes para esta tarea";
			}
		} else {
		...
		if (subtaskElements_NotCompleted.length == 0) {
			if (!spanish) {
				...
				subtaskContent = "No hay subtareas disponibles para completar.";
			}
		} else {
			subtaskContent = (
				<ul className={styles.taskContent}> {subtaskElements_NotCompleted} </ul>
			);
		}
		...
		if (subtaskElements_Completed.length == 0) {
			if (!spanish) {
				...
				completedSubtaskContent = "No hay subtareas completadas.";
			}
		} else {
			completedSubtaskContent = (
				<ul className={styles.taskContent}> {subtaskElements_Completed} </ul>
			);
		}
			...
			return (
				<div className="whitePageWrapper">
					...
					<main className="content">
						...
						<p className={styles.projectSubinfo}>
							Estado: {taskData.isCompleted}
						</p>
						{/* <p className={styles.projectSubinfo}>
                          Stage Start Date: 1/01/2023 | Project Start Date: 1/01/2023
                        </p> */}
						<Link href={`/project/${projectId}`}>Volver al Proyecto </Link>
						<h1>Subtareas</h1>
						<div className="tasklist" id="activetasklist">
							{subtaskContent}
						</div>
						<h1>Subtareas Completadas</h1>
						<div className="tasklist" id="completedtasklist">
							{completedSubtaskContent}
						</div>
						<div className={styles.boxtabContainer} id="photos-container">
						...
						{imageContent}
						<label>Elige una foto para subir</label>
						<input
							onChange={(e) => handleImageChange(e)}
							type="file"
							id="img"
